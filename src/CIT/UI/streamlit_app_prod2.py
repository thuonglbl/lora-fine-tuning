...
from CIT.RAGs.utils2 import RAGv3, VectorBase, get_one_answer_from_rag  # type: ignore
...
def main_app():
    ...
    names= st.experimental_user["name"].split(" ") if False else ["Test","User"]
    first_name=names[0]
...
if False and not st.experimental_user.is_logged_in:
    login_screen()
else:
    st.session_state.username = st.experimental_user["name"] if False else "test_user"
