...
def main_app():
    ...
    st.markdown(
        "RAG based on the [Corporate IT Knowledge Base](https://confluence.yourcompany.com/spaces/CORPORATEITKNOWLEDGEBASE/pages/1311588781/Corporate+IT+-+User+Knowledge+Base)"
    )  # add the link as an hyperlink
    st.markdown("Use the model first for its main purpose: **answering IT-related questions \
                at Acme Corp**, based on the [Corporate IT Knowledge Base](https://confluence.yourcompany.com/spaces/CORPORATEITKNOWLEDGEBASE/pages/1311588781/Corporate+IT+-+User+Knowledge+Base). Then, feel free to test\
...
def login_screen():
    ...
    st.subheader(f"Please log in with your {settings.COMPANY_NAME} account")
