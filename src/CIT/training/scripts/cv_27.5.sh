...
######################################################################
###############Evaluate the 70B base model############################
######################################################################
MODEL_NAME_70B="llama3.3:70b"
TEST_ANSWERS_FOLDER_BASE_70B="./src/CIT/training/models/cv/data/test_answers/ft_${PIPELINE_NAME}/base_70b"
SCORES_FOLDER_BASE_70B="./src/CIT/training/models/cv/scores/ft_${PIPELINE_NAME}/base_70b"
if true; then
python base_cv_eval.py \
  --model_name "$MODEL_NAME_70B" \
  --splits_folder "$TRAINING_OUTPUT_DIR" \
  --test_answers_folder "$TEST_ANSWERS_FOLDER_BASE_70B" \
  --scores_folder "$SCORES_FOLDER_BASE_70B" \
  --always_do_retrieval "$ALWAYS_DO_RETRIEVAL" \
  --add_external_links_docs "$ADD_EXTERNAL_LINKS_DOCS" \
  --answers_already_computed "$ANSWERS_ALREADY_COMPUTED"
fi
echo "Base 70B model evaluation completed."
