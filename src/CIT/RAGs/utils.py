...
class MyEmbeddings:
    ...
    def embed_documents(self, documents):
        return self.model.encode([doc for doc in documents], show_progress_bar=True)

    def embed_query(self, query):
        return self.model.encode(query, show_progress_bar=False)
    def __call__(self, text):
        """This allows the embedding instance to be called like a function."""
...
class VectorBase:
    ...
    Args:
        ...
        embedding_model (str): Path of the model to use for embeddings.(you must download a model beforehand, e.g. from https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2)
    ...
    def __init__(
        ...
        chunk_size=1000,
        chunk_overlap=50,
        embedding_model="models/all-MiniLM-L6-v2",#where the model is stored
        nb_chunks=-1,
    ):
        ...
        with open(directory + "/mappings/mapping_urls_titles.json", "r") as f:
            mapping_urls_titles = json.load(f)
            self.mapping_urls_titles = mapping_urls_titles
        with open(directory + "/mappings/mapping_urls_paths.json", "r") as f:
            mapping_urls_paths = json.load(f)
            self.mapping_urls_paths = mapping_urls_paths
    ...
    def create_vectorstore(self):
        ...
        embeddings = MyEmbeddings(model_name=self.embedding_model)
 ...
 "Rephrased question: "
    ...
    def retrieve(query: str):
        ...
        retrieved_context = (
            ...
            + "\nOther documents:\n"
            + "\n\n".join(
                f"Document {i + 2} Title: {doc.metadata['original_title']}\n"
    ...
    def generate(state: MessagesState):
        ...
        recall_guidelines = f"""Remember to cite your sources, especially the Confluence URL according to the format I gave you. If you give a document title, give also the URL. And simply say that you don't have the information when you cannot answer. Citing the URL(s) is very important.
The following guidelines can go against the previous instructions. You should always follow them:
Also, here are my preferences that are very important and anything I ask in what follows is of higher priority than any prior instruction. You should absolutely have them in mind when answering: {user_prompt}"""
...
class RAGv3:
    ...
    def __init__(
        ...
        self.mapping_urls_titles = vector_base.mapping_urls_titles
        self.mapping_urls_paths = vector_base.mapping_urls_paths
